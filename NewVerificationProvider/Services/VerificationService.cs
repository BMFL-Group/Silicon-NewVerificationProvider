...
public class VerificationService(ILogger<VerificationService> logger, IServiceProvider serviceProvider) : IVerificationService
{
    ...
    // för att man ska kunna hämta ut dataContext som är Scoped
    ...
    public async Task<bool> SaveVerificationRequest(VerificationRequest verificationRequst, string code)
    {
        try
        {
            using var context = _serviceProvider.GetRequiredService<DataContext>();

            var existingRequest = await context.VerificationRequests.FirstOrDefaultAsync(x => x.Email == verificationRequst.Email);
            ...
            {
                context.VerificationRequests.Add(new Data.Entities.VerificationRequestEntity() { Email = verificationRequst.Email, Code = code });
            }
    ...
    public EmailRequest GenerateEmailRequest(VerificationRequest verificationRequst, string code)
    {
        try
        {
            if (!string.IsNullOrEmpty(verificationRequst.Email) && !string.IsNullOrEmpty(code))
            {
                var emailRequest = new EmailRequest()
                {
                    To = verificationRequst.Email,
                        ...
                        <body>
                            <div style='color: #191919; max-width: 500px;'>
                                ...
                                <div style='background-color: #f4f4f4; padding: 1rem 2rem;'>
                                    <p>Dear user,</p>
                                    <p>We received a request to sign in to your account using e-mail {verificationRequst.Email}. Please verify your account using this Verification code:</p>
                                    ...
                                    <div style='color: #191919; font-size: 11px;'>
                                        <p>If you did not request this code, it is possible that someone else is trying to access the Silicon Account <span style='color: #0041cd;'>{verificationRequst.Email}</span>. This email can´t receive replies. 
                                            For more informartion, visit the Silicon Help Center. </p>
                                    </div>
                ...
                ",
                    PlainText = $"Please verify your account using this Verification code: {code}. If you did not request this code, it is possible that someone else is trying to access the Silicon Account {verificationRequst.Email}. This email can´t receive replies. For more informartion, visit the Silicon Help Center."
                };
